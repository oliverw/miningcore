...
namespace MiningForce.Blockchain.Bitcoin
{
    public static class BitcoinUtils
    {
	    ...
	    /// script the wallet knows how to spend; version 0x05 (these addresses begin with the digit '3')
	    /// 
	    /// Pay-to-pubkey-hash(p2pkh): payload is RIPEMD160(SHA256(ECDSA_publicKey)) where 
	    /// ECDSA_publicKey is a public key the wallet knows the private key for; version 0x00 
	    ...
	    public static IDestination AddressToScript(string address)
	    {
			...
			// skip first byte which is the version/application byte - https://en.bitcoin.it/wiki/Base58Check_encoding
		    var scriptId = new KeyId(decoded.Skip(1).ToArray());
		    return scriptId;
	    }
	}
}
