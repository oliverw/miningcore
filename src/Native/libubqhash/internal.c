...
bool static ethash_compute_cache_nodes_uip1(
	node* const nodes,
	uint64_t cache_size,
	ethash_h256_t const* seed
)
...
ethash_light_t ethash_light_new_internal(uint64_t cache_size, ethash_h256_t const* seed, bool uip1)
{
	...
	if (uip1) {
		if (!ethash_compute_cache_nodes_uip1(nodes, cache_size, seed)) {
...
ethash_light_t ethash_light_new(uint64_t block_number)
{
	...
	ethash_light_t ret;
        if (block_number >= ETHASH_EPOCH_LENGTH * UBQHASH_UIP1_EPOCH) {
		ret = ethash_light_new_internal(ethash_get_cachesize(block_number), &seedhash, true);
	} else {
		ret = ethash_light_new_internal(ethash_get_cachesize(block_number), &seedhash, false);
	}
