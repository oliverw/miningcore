...
public class BitcoinJobTests : TestBase
{
    ...
    public void Process_Valid_Block()
    {
        ...
        // validate & process
        var (share, blockHex) = job.ProcessShare(con, extraNonce2, nTime, nonce);
    ...
    public void Process_Duplicate_Submission()
    {
        ...
        Assert.ThrowsAny<StratumException>(()=> job.ProcessShare(con, extraNonce2, nTime, nonce));
    }
    ...
    private (BitcoinJob, StratumConnection) CreateJob()
    {
        ...
        var coin = ModuleInitializer.CoinTemplates["dash"] as BitcoinTemplate;
        ...
        var con = new StratumConnection(new NullLogger(LogManager.LogFactory), container.Resolve<RecyclableMemoryStreamManager>(), clock, "1", false);
