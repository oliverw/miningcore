...
namespace MiningCore.Tests.Blockchain.Monero
{
    public class BitcoinJobTests : TestBase
	{
		...
		StratumClient<MoneroWorkerContext> worker = new StratumClient<MoneroWorkerContext>
		{
			Context = new MoneroWorkerContext
			{
				Difficulty = 1000,
			}
		};
	    ...
	    public void MoneroJob_Should_Accept_Valid_Share()
	    {
		    ...
		    Assert.Equal(share.StratumDifficulty, 1000);
		}
