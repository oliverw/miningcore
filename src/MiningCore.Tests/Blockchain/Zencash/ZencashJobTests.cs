...
using MiningCore.Blockchain.Zencash.DaemonResponses;
...
{
    public class ZencashJobTests : TestBase
    {
        readonly PoolConfig poolConfig = new PoolConfig
        {
            Coin = new CoinConfig { Type = CoinType.ZEN }
        };
        ...
        [Fact]
        public void ZencashJob_Testnet_Validate_FoundersRewardAddress_At_Height()
        {
            var job = new ZencashJob();

            var bt = new ZencashBlockTemplate
            {
                ...
                Subsidy = new ZencashBlockSubsidy
                {
                    Founders = 2.5m,
                    Miner = 10m,
                }
